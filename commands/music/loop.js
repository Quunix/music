...
module.exports = {
    ...
    execute(client, message, args) {
        ...
        if (args.join('').toLowerCase() === 'queue') {
            ...
            .setTitle('Loop')
            .setColor('#8387DE')
            .setDescription(success ? `Döngü Modu:  **${queue.repeatMode === 0 ? 'Aktif Değil' : 'Aktif'}**
             Mevcut müzik durmadan tekrarlanacak
            (listedeki bütün müzikleri **${client.config.px}loop queue** seçeneği ile tekrarlata bilirsiniz.) 🔂` : `${message.author}, Birşeyler yanlış gitti. ❌`);
            return message.channel.send({ embeds: [loop] });
        };
    },
};